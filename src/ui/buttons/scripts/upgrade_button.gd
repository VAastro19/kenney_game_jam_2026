...
@export var upgrade: Enums.Upgrade
...
var cost: float:
	get: return economy_manager.calculate_upgrade_cost(cost_multiplier, cur_buys + 1, initial_cost_offset)
...
func _ready() -> void:
	...
	upgrade_type_name = str(Enums.Upgrade.keys()[upgrade].to_lower())
...
func _on_button_pressed() -> void:
	if is_unlocked:
		if cost <= economy_manager.get(cost_type_name + "_amount"):
			...
			_bought()
			if upgrade == Enums.Upgrade.MONUMENT:
				EventBus.OnMonumentBuilt.emit()
			else:
				economy_manager.set(upgrade_type_name + "_count", economy_manager.get(upgrade_type_name + "_count") + 1)
