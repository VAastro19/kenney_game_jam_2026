...
func _on_click(amount: float, type: Enums.CoinType) -> void:
	var coin_type_name: String = Enums.CoinType.keys()[type].to_lower()
...
func _on_tick() -> void:
	...
	EventBus.OnUpdateCoin.emit(Enums.CoinType.BLUE)

	green_amount = min(coin_cap, green_amount + green_production)
	EventBus.OnUpdateCoin.emit(Enums.CoinType.GREEN)

	yellow_amount = min(coin_cap, yellow_amount + yellow_production)
	EventBus.OnUpdateCoin.emit(Enums.CoinType.YELLOW)

	red_amount = min(coin_cap, red_amount + red_production)
	EventBus.OnUpdateCoin.emit(Enums.CoinType.RED)
